...
async function configureWebhook() {
  ...
  try {
    // Asegura migraciones antes de nada
    await migrate();

    // Obtiene info actual del webhook
    const info = await bot.getWebHookInfo();
    if (info.url === hookUrl) {
      console.log('📌 Webhook ya configurado en:', hookUrl);
    } else {
      // Registra el webhook nuevo
      const result = await bot.setWebHook(hookUrl);
      console.log('✅ Webhook configurado en:', hookUrl, result);
    }

    webhookConfigured = true;
  } catch (err) {
    // Si es un 429, reintenta tras el tiempo sugerido
    const status = err.response?.statusCode;
    const retryAfter = err.response?.body?.parameters?.retry_after;
    if (status === 429 && retryAfter) {
      console.warn(`⚠️ Rate limited. Reintentando en ${retryAfter}s...`);
      setTimeout(configureWebhook, retryAfter * 1000);
    } else {
