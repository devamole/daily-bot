...
// Configura el webhook antes de registrar handlers o enviar mensajes
(async () => {
  await bot.setWebHook(`${domain}/api/webhook/${process.env.SECRET}`);
  console.log(`Webhook configurado en: ${domain}/api/webhook/${process.env.SECRET}`);
})();
...
bot.on('message', async (msg) => {
  ...
  // 3b) Respuesta vespertina
    ...
    const { rows: morningRows } = await db.execute({
      sql: `SELECT text FROM messages WHERE user_id = ? AND type = 'morning' AND date(timestamp, 'unixepoch') = ? ORDER BY id ASC LIMIT 1`,
      args: [userId, today],
    });
    ...
    if (score === 100) {
      ...
      await db.execute({
        sql: `INSERT INTO pending_responses (user_id, daily_id, type) VALUES (?, ?, 'followup_question')`,
        args: [userId, dailyId],
      });
  ...
  // 3c) Procesa follow-up
    ...
    const { rows: pendingRows } = await db.execute({
      sql: `SELECT id FROM pending_responses WHERE user_id = ? AND daily_id = ? AND sent = FALSE ORDER BY id ASC LIMIT 1`,
      args: [userId, dailyId],
    });
    ...
    if (pendingRows.length) {
      ...
      await db.execute({
        sql: `INSERT INTO follow_up_responses (pending_id, text, timestamp) VALUES (?, ?, ?)`,
      ...
      const { rows: morningRows2 } = await db.execute({
        sql: `SELECT text FROM messages WHERE user_id = ? AND type = 'morning' AND date(timestamp,'unixepoch') = ? ORDER BY id LIMIT 1`,
        args: [userId, today],
      });
      const planText2 = morningRows2[0]?.text || '';
      const { rows: updateRows } = await db.execute({
        sql: `SELECT text FROM messages WHERE user_id = ? AND type = 'update' AND date(timestamp,'unixepoch') = ? ORDER BY id LIMIT 1`,
        args: [userId, today],
      });
  ...
  await db.execute({
    sql: `INSERT INTO messages (chat_id, user_id, message_id, text, timestamp, type) VALUES (?, ?, ?, ?, ?, ?)`,
